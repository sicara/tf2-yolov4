...
def YOLOv3_head(input_shapes, anchors, num_classes):
    ...
    y1 = conv_classes_anchors(input_1, num_anchors=len(anchors[2]), num_classes=num_classes)
    y2 = conv_classes_anchors(input_2, num_anchors=len(anchors[1]), num_classes=num_classes)
    y3 = conv_classes_anchors(input_3, num_anchors=len(anchors[0]), num_classes=num_classes)

    return tf.keras.Model([input_1, input_2, input_3], [y1, y2, y3])


def conv_classes_anchors(inputs, num_anchors, num_classes):
    """
    Applies successively Conv2D -> BN -> LeakyReLU

    Args:
        inputs (tf.Tensor): 4D (N,H,W,C) input tensor
        num_anchors (int): Number of anchors for the given output level
    ...
    x = tf.keras.layers.Conv2D(
        ...
        padding='same',
        use_bias=True
        # use_bias To doubleCheck. is false in yolov3 tf2 but is not specified in yolov4.cfg and true in pytorch yolov4
    )(inputs)
...
if __name__ == "__main__":
    yolov3_anchors = [np.array([(10, 13), (16, 30), (33, 23)], np.float32) / 416,
